        ...
        return (
            <div className="mx-auto p-4 cursor-pointer bg-cover bg-center w-full min-h-screen">
                <div className="p-8 mt-20 text-center">
                    <p className="text-xl text-gray-100 my-6">Sorry!! This feature is available only on laptop or desktop.</p>
                    <p className='text-white text-xl bg-black p-4'>{"But here is the demo on how it's shown on laptop or desktop 👇"}</p>
                </div>
                <div className="max-w-7xl mx-auto lg:p-6 lg:mt-44 w-full">
                    <div className='w-full justify-between flex flex-col lg:flex-row items-center'>
                        <div className='flex flex-col lg:flex-row gap-6 w-full justify-between'>
                            <div className='flex gap-4 lg:gap-10 h-full'>
                                <Image width={100} height={100} className='lg:w-32 lg:h-32 w-20 h-20 object-cover object-center rounded-2xl' src="/logo.png" alt='' />
                                <div className='flex flex-col items-center'>
                                    <h1 className='text-3xl lg:text-6xl font-sai my-2 text-white'>
                                        <span className='font-kumar text-yellow-600'>#</span>
                                        {"Your website name appears here"}
                                    </h1>
                                    <p className='text-2xl hidden lg:block text-white font-light'>{"Your website tagline appears here"}</p>
                                </div>
                            </div>
                            <p className='text-2xl lg:hidden text-white font-light'>{"Your website tagline appears here"}</p>
                            <div className='flex flex-col lg:flex-row items-center gap-4'>
                                <div className='lg:w-[240px] w-full justify-center text-xl flex items-center gap-4 font-semibold rounded-md border-2 border-white text-white p-4'>
                                    <p>Visits: {"Displays the count of clicks on your ad"}</p>
                                </div>
                                <Link href={'/about'} className='text-xl font-semibold w-full lg:w-[200px] text-center rounded-md bg-white text-gray-900 p-4 font-sai'>Visit Website</Link>
                            </div>
                        </div>
                    </div>
                    <hr className='w-full mt-10' />
                    <div className='my-10'>
                        <h1 className='text-white font-sai text-3xl'>What is {"(Your website name appears here)"}?</h1>
                        <p className='py-4 text-white text-xl whitespace-pre-line'>
                            {"Your website description appears here"}
                        </p>
                    </div>
                </div>
            </div>
